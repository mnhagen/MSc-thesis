...
A widely used definition of neural operators is given by Kovachki et al. \cite{neural-operators}, who state that a neural operator should satisfy three key properties:
...
The learned coordinate transformation is denoted by $\phi^{-1}$, implemented in the original work as the network IPHI. Given input coordinates $x$, and optionally a vector of geometry parameters $g$, the mapping is defined as
\begin{equation}
\phi^{-1}(x, g) = x + f(x, g),
\end{equation}
where $f(x, g)$ is represented by a lightweight multilayer perceptron and initialized close to zero so that the mapping begins near the identity transformation. The coordinate transform is trained jointly with the FNO and learns a mapping that minimizes the prediction error while adapting to the geometry of the problem.
